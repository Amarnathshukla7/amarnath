...
describe("Transaction.vue", () => {
  ...
  describe('Given valid cid in URL query', () => {
    ...
    it("should render the transaction page", () => {
      ...
      // Click on paynow
      cy.get('.v-input--radio-group__input > .v-item--active > .v-label').click();
      ...
      cy.get('.__PrivateStripeElement > iframe').then(($elements) => {
        ...
        const creditInput = $elements.eq(0).contents().find("input[data-elements-stable-field-name=cardNumber]");
        cy.wrap(creditInput).type("424242424242424212/50");

        // type into the cvc element.
        const cvcInput = $elements.eq(0).contents().find("input[data-elements-stable-field-name=cardCvc]");
        cy.wrap(cvcInput).type("12312345");
      });

      cy.get(':nth-child(1) > .v-input > .v-input__control > .v-input__slot > .v-input--selection-controls__input > .v-input--selection-controls__ripple').click();
